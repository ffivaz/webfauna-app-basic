...
@Component({
    ...
    templateUrl: 'structure-list.page.html'
})
export class StructureListPage implements OnInit {
    ...
    select(sel?: RealmDBModel) {
        if (sel) {
            this.observation.structureCode = sel['REST-ID'];
            this.nc.setRoot(InsertFormPage, {
                'obs': this.observation
            })
        } else if (sel === null) {
            this.observation.structureCode = null;
            this.nc.setRoot(InsertFormPage, {
                'obs': this.observation
            })
        }
    ...
    filterList() {
        this.rs.getRealmFromFile('tsp', this.searchTerm, this.lang)
    ...
    ngOnInit() {
        ...
        this.rs.getRealmFromFile('tsp', null, this.lang)
            .then(response => {
                this.list = response;
            });
