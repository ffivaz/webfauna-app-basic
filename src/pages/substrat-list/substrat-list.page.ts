...
@Component({
    selector: 'page-substrat-list',
    templateUrl: 'substrat-list.page.html'
})
export class SubstratListPage implements OnInit {
    ...
    select(sel?: RealmDBModel) {
        if (sel) {
            this.observation.substratCode = sel['REST-ID'];
            this.nc.setRoot(InsertFormPage, {
                'obs': this.observation
            })
        } else if (sel === null) {
            this.observation.substratCode = null;
            this.nc.setRoot(InsertFormPage, {
                'obs': this.observation
            })
        }
    ...
    filterList() {
        this.rs.getRealmFromFile('sbt', this.searchTerm, this.lang)
    ...
    ngOnInit() {
        ...
        this.rs.getRealmFromFile('sbt', null, this.lang)
            .then(response => {
                this.list = response;
            });
    }
